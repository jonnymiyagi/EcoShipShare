...
//  EcoShipShare
//
//  Created by JŌNNY MĪYĀGĪ on 7/26/21.
//  Copyright © 2021 JŌNNY MĪYĀGĪ. All rights reserved.
//
...
UIImagePickerControllerDelegate, MCSessionDelegate, MCBrowserViewControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        navigationItem.leftBarButtonItem = UIBarButtonItem(barButtonSystemItem:
            .add, target: self, action: #selector(showConnectionPrompt))
        
        navigationItem.rightBarButtonItem = UIBarButtonItem(barButtonSystemItem:
            .camera, target: self, action: #selector(importPicture))
        ...
        if mcSession.connectedPeers.count > 0 {
            if let imageData = image.pngData() {
                do {
                    ...
                    let ac = UIAlertController(title: "Send error", message:
